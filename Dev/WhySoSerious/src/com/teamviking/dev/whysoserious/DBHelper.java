...
public class DBHelper extends SQLiteOpenHelper {
	...
	private static final String DATABASE_CREATEJOKETABLE = "create table "
			...
			+ COLUMN_MONOLOGUETEXT + " text,"
			+ COLUMN_RATINGSCALE + " int2 not null,"
			+ COLUMN_COMMENTS + " text not null,"
			+ COLUMN_JOKESOURCE + " text not null,"
			+ COLUMN_RELEASESTATUS + " tinyint not null,"
     ...
     * Remember the database is not actually created or opened until one of getWritableDatabase() 
...
//	public long insertNewPlayer(String firstName, String lastName, String sportName, int score) {
//		ContentValues cv = new ContentValues();
//		cv.put(COLUMN_FIRST_NAME, firstName);
//		cv.put(COLUMN_LAST_NAME, lastName);
//		cv.put(COLUMN_SPORT, sportName);
//		cv.put(COLUMN_SCORE, score);
//		
//		long code = getWritableDatabase().insert(TABLE_SCORES, null, cv);
//		return code;
//	}

	public long insertNewJoke(String category, String subCategory, int jokeTypeCode, 
	...
	public Cursor getJokes() {
		return getWritableDatabase().query(TABLE_JOKE, null, null, null, null, null, null);
	}
	
	public Cursor getFiveRandomJokes() {
		return getWritableDatabase().query(TABLE_JOKE+" Order BY RANDOM() LIMIT 5",
                new String[] { "*" }, null, null, null, null, null);
	}
	
	public String getFirstJokeDate()
	{
		Cursor cursor= getReadableDatabase().rawQuery("SELECT createDate FROM jokeTable ORDER BY ROWID ASC LIMIT 1",null);
		cursor.moveToFirst();
		String firstJokeDate = cursor.getString(cursor.getColumnIndex(DBHelper.COLUMN_CREATEDATE));
		cursor.close();
	...
	{
		Cursor cursor= getReadableDatabase().rawQuery("SELECT createDate FROM jokeTable ORDER BY ROWID DESC LIMIT 1",null);
		cursor.moveToFirst();
		String lastJokeDate = cursor.getString(cursor.getColumnIndex(DBHelper.COLUMN_CREATEDATE));
		cursor.close();
	...
	public Integer getNumberOfCategoryJokesInDatabase(String category) {
		Cursor mCount= getReadableDatabase().rawQuery("select count(*) from jokeTable where category = '" + category+"'", null);
		mCount.moveToFirst();
	...
	public Cursor getCategories() {
		return getWritableDatabase().query(TABLE_CATEGORY, null, null, null, null, null, null);
	}
	/*
	 * deleteJoke()
	 * 
	...
	public void deleteJoke(int id) {
		getWritableDatabase().delete(TABLE_JOKE, COLUMN_ID + "=?", new String[] { String.valueOf(id) });
	}
	...
	public void deleteCategory(int id) {
		getWritableDatabase().delete(TABLE_CATEGORY, COLUMN_ID + "=?", new String[] { String.valueOf(id) });
	}
//	/*
//	 * get40()
//	 *  
//	 * C*R*UD	Retrieve
//	 * 
//	 * method written to return the set of student records who's grade is exactly 40
//	 * for demonstration purposes, not necessarily a useful method
//	 */
//	public Cursor get40() {
//		return getWritableDatabase().query(TABLE_SCORES, null, COLUMN_SCORE + "=?", new String[] { "40" }, null, null, null);
//	}
